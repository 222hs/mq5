...
input int    TakeProfit     = 25;      // Take Profit (points x10 = pips on gold)
input int    StopLoss       = 35;      // Stop Loss
...
double pipFactor;   // 1 pip in points for gold
...
int OnInit()
  {
   ...
   if(_Period!=PERIOD_M1 && _Period!=PERIOD_M5)
      Print("Warning: EA designed for M1/M5, current TF: ",EnumToString(_Period));
   ...
   // pip on gold: 1 pip = 10 points when digits are 2/3-style quotes
...
void OnTick()
  {
   ...
   // series order: index [n-1] = current when ArraySetAsSeries not used; CopyBuffer returns oldest first
   double rsiCur = rsi[1];
...
//+------------------------------------------------------------------+
//| Log closed deals (TP/SL) of this EA                              |
//+------------------------------------------------------------------+
   ...
   if((ENUM_DEAL_ENTRY)HistoryDealGetInteger(trans.deal,DEAL_ENTRY)==DEAL_ENTRY_OUT)
     {
      ...
      PrintFormat("POSITION CLOSED | Deal=%I64u Profit=%.2f Reason=%s",
                  trans.deal,profit,
                  (ENUM_DEAL_REASON)HistoryDealGetInteger(trans.deal,DEAL_REASON)==DEAL_REASON_TP ? "TakeProfit" :
                  (ENUM_DEAL_REASON)HistoryDealGetInteger(trans.deal,DEAL_REASON)==DEAL_REASON_SL ? "StopLoss" : "Other");
     }
  }
//+------------------------------------------------------------------+
int CountPositions()
