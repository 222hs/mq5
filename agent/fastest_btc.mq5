...
bool   g_earlyEntry        = false; // دخول مبكر: يركب الشمعة الحالية من بدايتها
double g_earlyMomATR       = 0.3;   // مقدار تحرّك الشمعة (×ATR) لتفعيل الدخول المبكر
...
double g_autoSLATR = 2.0;  // مضاعف ATR للستوب (ذهب 1.0 · بتكوين 2.0) — قابل للتعديل
...
void WriteCurrentSettings()
  {
   ...
   j += "  \"EarlyEntry\": "   + (g_earlyEntry ? "1" : "0")          + ",\n";
   j += "  \"EarlyMomATR\": "  + DoubleToString(g_earlyMomATR,2)     + ",\n";
...
void LoadNewsBlock()
  {
   int fh = FileOpen("BSX_NewsBlock.txt", FILE_READ|FILE_TXT|FILE_COMMON);
...
void WriteStatus(const string s)
  {
   int fh = FileOpen("BSX_Status.txt", FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_COMMON);
   if(fh == INVALID_HANDLE) return;
...
void LoadSettings()
  {
   ...
   bool   early  = (ReadSetting("EarlyEntry",       0.0) > 0.5);
   double earlyM = ReadSetting("EarlyMomATR",       0.3);
   ...
   g_earlyEntry=early; g_earlyMomATR=MathMax(0.05,earlyM);
...
void OnTick()
  {
   ...
   bool hftMode = (g_cooldownSecs < 30) || g_earlyEntry; // الدخول المبكر يقيّم كل تِك
   if(!hftMode)
     {
   ...
   // ── إشارة الدخول: مبكر (الشمعة الحالية) أو مؤكّد (الشمعة المغلقة) ──
   bool bullBar, bearBar;
   if(g_earlyEntry)
     {
      // يركب الشمعة الحالية أول ما تتحرّك بمقدار earlyMomATR × ATR
      double upMove = c[0]-o[0], dnMove = o[0]-c[0];
      bullBar = (upMove >= g_earlyMomATR*atr1);
      bearBar = (dnMove >= g_earlyMomATR*atr1);
     }
   else
     {
      bullBar = (c[1] > o[1]) && ((c[1]-o[1])/(h[1]-l[1]+1e-10) >= 0.25) && (h[1]-l[1]) <= 5.0*atr1;
      bearBar = (c[1] < o[1]) && ((o[1]-c[1])/(h[1]-l[1]+1e-10) >= 0.25) && (h[1]-l[1]) <= 5.0*atr1;
     }
...
// يقرأ مستويات كلود من BSX_GridLevels.txt (صيغة: BUY:p,p\nSELL:p,p)
void ReadAIGridLevels()
  {
   g_aiBuyN = 0; g_aiSellN = 0;
   int fh = FileOpen("BSX_GridLevels.txt", FILE_READ|FILE_TXT|FILE_ANSI|FILE_COMMON);
   if(fh == INVALID_HANDLE) return;
